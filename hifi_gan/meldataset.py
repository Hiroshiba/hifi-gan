...
def resample(
    array: torch.Tensor, src_rate: int, tgt_rate: int, index=0, length: int = None
):
    if length is None:
        length = int(array.size(1) / src_rate * tgt_rate)
    indexes = (torch.rand(1) + index + torch.arange(length, dtype=torch.float32)) * (
        src_rate / tgt_rate
    )
    ...
    def _get_files(x: str):
        ...
        if not os.path.exists(wav_path):
            wav_path = os.path.join(
                a.input_wavs_dir, wav_path + (".npy" if a.input_wavs_npy else ".wav")
            )
...
class MelDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        training_files: List[Tuple[str, Optional[str], Optional[str]]],
        segment_size,
        n_fft,
        num_mels,
        hop_size,
        ...
        if not self.fine_tuning:
            if self.split:
                if audio.size(1) >= self.segment_size:
                    ...
                    audio_start = (
                        random.randint(0, max_audio_start) if max_audio_start > 0 else 0
                    )
                    ...
                    if f0 is not None:
                        ...
                        f0 = torch.nn.functional.pad(
                            f0, (0, frames_per_seg - f0.size(1)), "constant"
                        )
            else:
                if f0 is not None:
                    f0 = resample(f0, f0_rate, self.sampling_rate / self.hop_size)
            ...
            if self.split:
                ...
                if audio.size(1) >= self.segment_size:
                    ...
                    mel_start = (
                        random.randint(0, max_mel_start) if max_mel_start > 0 else 0
                    )
